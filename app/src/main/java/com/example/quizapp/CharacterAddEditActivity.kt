...
/**
 * Creation / modification d'un personnage.
 * Mode edition si l'intent contient un "CHARACTER_ID" different de 0.
 * L'utilisateur peut choisir une image (depuis le telephone) ou n'en mettre aucune.
 */
class CharacterAddEditActivity : AppCompatActivity() {

    private lateinit var dbHelper: DatabaseHelper
    private lateinit var imagePreview: ImageView
    private var editingId: Int = 0
    private var existing: Character? = null
    private var selectedImagePath: String = ""   // vide = pas d'image choisie

    // Selecteur d'image : l'image choisie est copiee dans le stockage de l'app
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editingId = intent.getIntExtra("CHARACTER_ID", 0)
        if (editingId != 0) {
            existing = dbHelper.getCharacterById(editingId)
            existing?.let {
                title.text = "Modifier le personnage"
        ...
        btnSave.setOnClickListener {
            ...
            if (editingId == 0) {
                ...
                Toast.makeText(this, "Personnage ajoute", Toast.LENGTH_SHORT).show()
            } else {
                dbHelper.updateCharacter(character)
                Toast.makeText(this, "Personnage modifie", Toast.LENGTH_SHORT).show()
            }
            finish()
        }
    }

    /** Copie l'image choisie dans le stockage interne et renvoie son chemin absolu. */
    private fun copyImageToInternal(uri: Uri): String? {
        return try {
            val input = contentResolver.openInputStream(uri) ?: return null
            val file = File(filesDir, "char_${System.currentTimeMillis()}.jpg")
